...
class LspClient(object):
    ...
    def initialize(self, processId, rootPath, rootUri, initializationOptions, capabilities, trace, workspaceFolders):
        ...
        2. Notifications should be dropped, except for the exit notification. This will allow
        ...
        send any additional requests or notifications to the server.
        In addition the server is not allowed to send any requests or notifications to the client until it has responded
        with an InitializeResult, with the exception that during the initialize request the server is allowed
        to send the notifications window/showMessage, window/logMessage and telemetry/event
        ...
        :param str rootPath: The rootPath of the workspace. Is null if no folder is open.
            Deprecated in favour of rootUri.
        :param DocumentUri rootUri: The rootUri of the workspace.
        ...
        :param list workspaceFolders: The workspace folders configured in the client when the server starts.
    ...
    def initialized(self):
        ...
        received the result of the initialize request
    ...
    def shutdown(self):
        ...
        the result of the initialize request but before the client is sending any other request or notification
        to the server. The server can use the initialized notification for example to dynamically register capabilities.
        The initialized notification may only be sent once.
        """
    ...
    def exit(self):
        ...
        client received the result of the initialize request but before the client is sending any other request
    ...
    def didOpen(self, textDocument):
        ...
        truth using the document's uri. Open in this sense means it is managed by the client.
        ...
        followed by a textDocument/didOpen with the new language id if the server
    ...
    def documentSymbol(self, textDocument):
        """
        The document symbol request is sent from the client to the server to return a flat list of all symbols found
        in a given text document. Neither the symbol's location range nor the symbol's container name should be used
        to infer a hierarchy.
    ...
    def definition(self, textDocument, position):
        """
        The goto definition request is sent from the client to the server to resolve the definition location
    ...
    def completion(self, textDocument, position, context):
        ...
        :param CompletionContext context: The completion context.
                    This is only available if the client specifies
